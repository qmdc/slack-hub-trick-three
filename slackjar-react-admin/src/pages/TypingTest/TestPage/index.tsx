...
import { getRandomArticle, saveTestRecord, type TypingArticle, type SaveTestRecordRequest } from '../../../apis/modules/typingtest';
...
export default function TypingTestPage() {
 ...
 <Col span={6}>
 <Statistic title="当前WPM" value={stats.wpm} prefix={<TrophyOutlined className="text-yellow-500"/>}/>
 </Col>
 <Col span={6}>
 <Statistic title="准确率" value={stats.accuracy} suffix="%" prefix={<TagOutlined className="text-green-500"/>}/>
 </Col>
 <Col span={6}>
 <Statistic title="用时" value={isStarted ? Math.round((currentTime - startTime) / 1000) : 0} suffix="秒" prefix={<ClockCircleOutlined className="text-blue-500"/>}/>
 </Col>
 <Col span={6}>
 ...
 <Button type="primary" icon={isStarted ? <RestOutlined/> : <PlayCircleOutlined/>} onClick={loadArticle} disabled={isStarted && !isFinished}>
 {isStarted ? '重新开始' : '换一篇'}
 </Button>
 </Space>
 </div>
 ...
 {!isStarted ? (<Button type="primary" size="large" icon={<PlayCircleOutlined/>} onClick={handleStart} className="w-full h-16 text-lg">
 开始测试
 </Button>) : (<textarea ref={inputRef} value={userInput} onChange={handleInputChange} className="w-full h-32 p-4 text-lg font-mono border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="在这里输入..." autoComplete="off" autoCorrect="off" autoCapitalize="off" spellCheck={false}/>)}
 </Card>) : null}

 {showResult && resultData && (<Card title="测试结果" bordered={false} className="mt-6 bg-gradient-to-r from-blue-50 to-indigo-50">
 <Row gutter={16}>
 <Col span={8}>
 <Statistic title="WPM (每分钟字数)" value={resultData.wpm} prefix={<TrophyOutlined className="text-yellow-500"/>} className="text-center"/>
 </Col>
 <Col span={8}>
 <Statistic title="准确率" value={resultData.accuracy} suffix="%" prefix={<TagOutlined className="text-green-500"/>} className="text-center"/>
 </Col>
 <Col span={8}>
 <Statistic title="用时" value={resultData.duration} suffix="秒" prefix={<ClockCircleOutlined className="text-blue-500"/>} className="text-center"/>
 </Col>
 </Row>
 ...
 <Col span={8}>
 <Statistic title="错误字符" value={resultData.totalChars - resultData.correctChars} className="text-center"/>
 </Col>
 </Row>
